...
def realtime(tortue=[Kabuki()], pas=0.1, eps=1, prop=1, inte=1):
    ...
    while 1:
        ...
        tortue[1].rejoidre2(tortue[0].x+(2*tortue[0].ecart)*(-np.cos(alpha)+np.sin(alpha)), tortue[0].y+(2*tortue[0].ecart)*(-np.cos(alpha)-np.sin(alpha)), alpha, eps, prop, inte, pas)

        tortue[2].rejoidre2(tortue[0].x+(2*tortue[0].ecart)*(-np.cos(alpha)-np.sin(alpha)), tortue[0].y+(2*tortue[0].ecart)*(np.cos(alpha)-np.sin(alpha)), alpha, eps, prop, inte, pas)
